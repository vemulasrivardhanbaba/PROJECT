...
<%
	String owner=(String)application.getAttribute("owner");
	   ...
	   if ( rs2.next() )
		  ...
		  %>
<h1>  <%=file%>  ---- <%=block %> ---  is Safe</h1><a href="VerifyBlock.jsp">Back</a></br>

		  <%
		   }
		   else
		   {%>
		   <h1> <%=file%>  ---- <%=block %> ---is Not Safe</h1><a href="VerifyBlock.jsp">Back</a>Do you Want to recover <a href="recover.jsp?owner=<%=owner%>&block=<%=block%>&file=<%=file%>">Recover</a><%
		 //  String ss="attacker";
		  // application.setAttribute("fname",file);
		  //application.setAttribute("attacker",ss);
		   }
		   ...
		   {%>
		   <h1> File doesnot exist</h1><a href="VerifyBlock.jsp">Back</a><%
		   }
